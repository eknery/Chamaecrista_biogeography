...
## for coalescence
dir_out = "1_ingroup_sequences/for_coalescence/"
for(i in 1:length(my_fasta_list) ){
  locus_name = names(my_fasta_list)[i]
  one_marker = my_fasta_list[[i]]
  names(one_marker) = paste(names(my_fasta_list[[i]]), locus_name, sep="_")
  fasta_name = paste0(names(my_fasta_list)[i], ".fasta")
